...
AI训推平台 → 通用通信大模型训练 → 九天全能小助手 → Omega智能体平台 → 多平台大模型应用适配；
...
```markdown
**项目名称**：AI训推平台
**技术栈：** Python、CUDA/CANN、FastAPI、Docker、vLLM/SGLang/MindIE、LLM微调与蒸馏
**项目描述：**
主导设计并搭建集数据处理、多算法微调、模型蒸馏及服务部署于一体的一站式大模型工作流平台。平台降低了底层算力与代码使用门槛，支持业务人员自主进行模型定制与上线，将模型交付周期由3天缩短至1天以内，提升了团队的交付效率。
**核心工作：**
    1. **数据与训练流构建**：开发自动化数据标注与教师模型蒸馏工具，提纯专业领域数据；封装PT、SFT、DPO等训练范式，适配全量及LoRA/QLoRA轻量化微调，兼容不同算力配置。
    2. **推理部署优化**：集成vLLM、SGLang、MindIE等主流推理引擎，支持微调模型的一键上线、灰度发布与性能监控。
    3. **平台化与工具链开发**：基于FastAPI与Docker容器化技术构建RESTful接口，实现数据管理、训练任务发起及模型版本控制的全流程可视化操作。
```
...
```markdown
**项目名称**：AI训推平台
**技术栈：**Python、CUDA、CANN、FastAPI、Docker、LLMs、模型蒸馏、多类型微调算法
**项目描述：**搭建集数据处理、模型训练、模型蒸馏、服务部署于一体的一站式大模型训推平台，屏蔽底层算力与代码门槛，赋能业务人员快速完成模型定制化训练与上线，支撑内部多业务线模型迭代需求；将传统训推交付周期由3天缩短至小于1天，整体交付效率提升60%。
**核心工作：**
    1. **数据处理模块**：自研智能化数据标注工具，支持自动化生成高质量问答对；引入教师模型实现数据蒸馏，提纯专业化思维数据，从源头提升模型训练基础数据质量；
    2. **多范式训练模块**：封装PT预训练、SFT监督微调、DPO偏好训练、模型蒸馏四类训练范式；适配全量微调、LoRA、QLoRA轻量化微调方案，适配不同算力资源场景；
    3. **推理部署优化**：集成vLLM、SGLang、MindIE主流推理引擎，实现微调模型一键上线、灰度测试与性能监控，大幅简化模型上线流程；
    4. **工具链开发**：基于FastAPI封装RESTful标准化接口，结合Docker容器化技术，实现数据上传、训练任务创建、模型版本管理全流程页面化操作。
```
...
**项目名称**：通信领域通用大模型训练与对齐优化
**技术栈：**PyTorch、QLoRA、LoRA、Post-Train、SFT、DPO、分布式训练、模型评测体系
**项目概述**：针对通信行业算网运维、通信故障诊断、招投标分析、财务稽核等核心场景，基于开源大模型开展领域化训练与能力对齐，构建覆盖“领域知识注入→指令理解→偏好对齐”的全流程训练体系；最终相较原生开源底座模型，业务场景问答准确率提升15%，模型幻觉率降低7%，故障排查场景任务完成率提升12%。
**核心职责**：
...
```markdown
项目名称：多平台大模型应用适配
技术栈：Python、GPU、NPU、DCU、Docker、LLM、多模态模型、Embedding/Reranker
项目描述：针对GPU/NPU/DCU异构计算环境，完成大语言 10 余个模型的跨平台适配、微调与部署，构建统一模型管理与容器化部署体系，支撑内部多项AI业务落地。
核心工作： 
1.	完成昇腾 NPU 平台模型训推适配、算子优化，达成全流程国产化落地；
2.	构建一体化模型管理体系，统一管控大模型、多模态、向量类模型；
3.	研发容器化一键部署方案，优化上线流程，将模型部署周期由天级缩短至小时级，降低了后期运维成本。
```
...
```markdown
**项目名称**：多平台大模型应用适配
**技术栈**：Python、CUDA、CANN、MindIE、Docker、LLM、多模态模型、Embedding/Reranker
**项目描述**：面向GPU/NPU/CPU三类异构硬件环境，完成大模型、多模态模型、语音模型的跨平台适配、微调与统一管理；搭建容器化一键部署体系，解决国产硬件适配难、模型管理混乱、部署成本高的行业痛点；累计完成10+不同类型模型跨硬件适配，支撑多项内部AI业务落地。
**核心工作** ：
    1、国产NPU适配**：基于昇腾910B硬件平台，依托MindIE推理框架，完成各类大模型推理适配、算子兼容调试；同时适配CANN架构，落地轻量化微调开发，实现模型训推全流程国产化；
    2、统一模型管理平台：整合语言大模型、多模态模型、Embedding向量模型、Reranker重排模型、语音视频模型，实现模型上架、版本管理、权限管控、算力分配一体化管理；
    3、容器化一键部署：基于Docker封装标准化部署镜像，适配Windows/Linux多系统架构，实现任意硬件平台模型一键部署，大幅降低运维与上线成本；模型部署周期从1天缩短至 30分钟，运维人力成本减少一半。
```
...
2. 基于 nanobot 优化
3. 用**规则式动态**挑选few-shot 示例（few-shot数量不固定） 
4. 分类任务使用 Multi-class GRPO 训练获取分类边界定义，先根据人为经验修改提示词，使用测试数据跑结果，将分类错误的数据保存，用于grpo训练使用。
