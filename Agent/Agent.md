...
致命伤：如果强行用 BN，它会依赖 Batch 中其他样本的统计信息，这在分布式训练（数据并行）下，变相让当前样本窥探到了其他样本的全局分布，破坏了“样本独立性”假设。LN 每个样本独立计算，天然符合独立同分布（i.i.d）要求。
...
###### Q20：阶段二：自主检索（Autonomous Retrieval）
...
###### Q21：阶段三：多路径推理（Multi-path Reasoning）
...
```markdown
不是从零实现算法本身，而是做了**训练范式工程封装 + pipeline标准**：
